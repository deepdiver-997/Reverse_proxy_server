...
所以 QUIC 侧的 `socket_` 必须是 **listener/engine 级共享资源**，绝不能放进 `QuicTransportSession`（那是每连接视角）。Model A（design-quic-demux.md）把"收包的 socket"再上提一级，归 **ingress 的 demux** 所有，engine 连 socket 都没有：

```
QuicPacketDemux      (1, ingress)  共享 UDP socket (1 fd) —— 收所有人包、按 DCID 查 CID 表 → post 给 worker
QuicServerEngine     (1/worker)    engine_ + tick_timer_ + live_sessions_（无 socket！包由 demux post 进来）
QuicClientEngine     (0~1/worker)  engine_ + 自己的 UDP socket + tick_timer_（H3 上游，懒建，见 ADR-9）
...
- **收包**：你调 `lsquic_engine_packet_in()`（server 模式 `peer_ctx` 传 NULL，解复用完全靠 CID）。Model A 里收包发生在 **ingress 的 demux 线程**（共享 UDP socket），按 CID 路由后 `io_context::post` 到目标 worker，`QuicServerEngine::deliver_packet()` 在 worker 线程上调 `lsquic_engine_packet_in()`——引擎始终单线程驱动（见 §5）。
...
**把 C++ 的 `QuicTransportSession*` 直接当 conn_ctx 是标准做法**（lsquic 从不解引用它）。"每次新建"是因为每个新连接都需要一份全新的 session 状态。关键约束是**生命周期**：conn_ctx 必须在连接的最后一次回调（`on_conn_closed`）执行期间仍然有效——因此 session **自持**一份 `shared_ptr`（`self_`，`on_new_conn` 里 `adopt_self()`），`on_conn_closed` 里先取保活拷贝、再 `release_self()`，析构发生在该回调返回之后、恰好是 lsquic 停止引用 conn 的时刻。

`QuicServerEngine` 额外维护一个**弱引用注册表** `live_sessions_`（`on_new_conn` 登记、`on_conn_closed` 的 closed_cb 擦除），**只为优雅关闭**（停服时逐条 GOAWAY / CONNECTION_CLOSE，见 design-quic-demux.md §11.5）——不参与正常生命周期，正常关闭仍靠自持 shared_ptr。
...
- **已知缺口**：vendored lsquic 4.7.0 有两个握手互操作缺陷（无 SNI → `CERT_CB_ERROR`；Initial 未填充 1200），已用 `docs/lsquic-4.7.0-http3-interop-a-b.patch` 修复（构建钩子 `ensure_lsquic_patch.sh` 自动维护），报告见 [lsquic-handshake-bug-report.md](lsquic-handshake-bug-report.md)。
